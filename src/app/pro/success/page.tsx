...
const SuccessPageContent = () => {
	...
	const benefits = isYearly
		...
		: ['Access to all premium courses', 'Priority support', 'Exclusive community features', 'Monthly live Q&A sessions'];

	return (
		<div className='container mx-auto px-4 py-8 md:py-16 max-w-4xl h-screen'>
			<Card className='w-full overflow-hidden'>
				<div className={`w-full h-2 ${isYearly ? 'bg-gradient-to-r from-purple-500 to-pink-500' : 'bg-blue-500'}`} />
						...
						<ul className='space-y-3'>
							{benefits.map((benefit, index) => (
								<li
									...
									<Check className={`flex-shrink-0 h-6 w-6 ${isYearly ? 'text-purple-500' : 'text-green-500'}`} />
									<span>{benefit}</span>
								</li>
							))}
						</ul>
				...
				<CardFooter className='flex flex-col space-y-4'>
					...
					<p className='text-sm text-muted-foreground text-center'>Excited to get started? Explore your new Pro features now!</p>
				</CardFooter>
			</Card>
		</div>
	);
