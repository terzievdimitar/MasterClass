...
const BillingPage = () => {
	...
	const handleManageBilling = async () => {
		...
		try {
			...
			else throw new Error('Failed to create billing portal', url);
		} catch (error: any) {
				...
				{subscription ? (
					<>
						<CardHeader className='pb-0'>
							...
							<CardDescription className='text-gray-600'>Manage your subscription details below</CardDescription>
						</CardHeader>
						...
						<CardContent className='space-y-6 pt-6'>
							<div className='grid grid-cols-2 gap-4 bg-gray-50 p-4 rounded-lg'>
								<div>
									...
									<p className='text-lg font-semibold text-gray-800 capitalize'>{subscription.planType}</p>
								</div>

								<div>
									<p className='text-sm font-medium text-gray-500'>Status</p>
									<p className='text-lg font-semibold text-gray-800 capitalize'>{subscription.status}</p>
								</div>
								<div className='col-span-2'>
									<p className='text-sm font-medium text-gray-500'>Next billing date</p>
									<p className='text-lg font-semibold text-gray-800'>{formatDate(subscription.currentPeriodEnd)}</p>
								</div>
							</div>
							{subscription.cancelAtPeriodEnd && (
								<div className='flex items-center bg-yellow-50 p-4 rounded-lg text-yellow-700'>
									<AlertTriangle className='h-5 w-5 mr-3 flex-shrink-0' />
									<p className='text-sm'>Your subscription will be cancelled at the end of the current billing period.</p>
								</div>
							)}
						</CardContent>
					...
					<>
						...
						<CardHeader>
							...
							<CardDescription className='text-gray-600'>Upgrade to Pro to unlock premium features</CardDescription>
						</CardHeader>
						<CardContent className='text-center py-12'>
							<p className='text-lg mb-6 text-gray-700'>Get access to exclusive content and features with our Pro plan.</p>
							<Link href='/pro'>
